...
/**
 * Loads the "beaten games" analysis data from public/data/beaten_games.json —
 * per-game achievement "story markers" and per-win beaten status, generated
 * by a sibling scraper pipeline. Returns null if the file doesn't exist yet
 * or fails to parse, so callers (the /verification page) can degrade
 * gracefully instead of failing the build.
 */
export async function getBeatenGames(): Promise<BeatenGamesData | null> {
  if (typeof window !== 'undefined' || process.env.NODE_ENV === 'development') {
    try {
      const baseUrl = getBaseUrl()
      const response = await fetch(`${baseUrl}/data/beaten_games.json`)
      if (!response.ok) return null
      return await response.json()
    } catch {
      return null
  ...
  try {
    const { readFileSync } = await import('fs')
    const { join } = await import('path')
    const filePath = join(process.cwd(), 'public', 'data', 'beaten_games.json')
    return JSON.parse(readFileSync(filePath, 'utf8'))
  } catch {
    return null
  }
...
/**
 * Loads the "I Play Bro" Discord-thread matches from public/data/ipb_discord.json,
 * generated by a sibling scraper pipeline. Returns null if the file doesn't
 * exist yet or fails to parse, so callers (the /verification page) can
 * degrade gracefully instead of failing the build.
 */
export async function getIpbDiscord(): Promise<IpbDiscordData | null> {
  if (typeof window !== 'undefined' || process.env.NODE_ENV === 'development') {
    try {
      const baseUrl = getBaseUrl()
      const response = await fetch(`${baseUrl}/data/ipb_discord.json`)
      if (!response.ok) return null
      return await response.json()
    } catch {
      return null
  ...
  try {
    const { readFileSync } = await import('fs')
    const { join } = await import('path')
    const filePath = join(process.cwd(), 'public', 'data', 'ipb_discord.json')
    return JSON.parse(readFileSync(filePath, 'utf8'))
  } catch {
    return null
  }
